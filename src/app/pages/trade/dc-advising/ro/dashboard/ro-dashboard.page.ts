...
@Component({
  ...
  templateUrl: './ro-dashboard.page.html',
})
export class RODashboardPageComponent implements OnInit {
  stats = {
    total: 0,
    pending: 0,
    actionRequired: 0,
  ...
  calculateStats() {
    ...
    // For RO, pending means tasks waiting for their validation
    this.stats.pending = this.requests.filter(r => ['SUBMITTED', 'RO_VALIDATION'].includes(r.status)).length;
    // Action required might be returned cases or high priority
    this.stats.actionRequired = this.requests.filter(r => r.status === 'RO_VALIDATION').length;
